...
We identified many variables that have a characteristic which make it either not interesting to analyze (only one/ very few distinct values, text strings) or are not used in the analysis later on. 
...
We now only have 39 columns left, which make the data set easier to handle. In a next step we will adjust the type of some variables so that we can actually can work with the data more easily. 
