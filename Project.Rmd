...
model2 <- lm(log(price) ~ as.factor(accommodates) + bedrooms, data=train)
...
model2 <- lm(log(price) ~ as.factor(accommodates) + cent_dist, data=train)
...
model3 <- lm(log(price) ~ as.factor(accommodates) + cent_dist + room_type, data=train)
...
model4 <- lm(log(price) ~ as.factor(accommodates) + cent_dist + room_type + bedrooms + bathrooms + review_scores_rating, data=train)
...
```{r}
full.model <- lm(log(price) ~., data = train)
